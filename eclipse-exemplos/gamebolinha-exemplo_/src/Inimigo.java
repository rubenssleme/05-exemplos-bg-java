...
public class Inimigo {
	...
	public Inimigo(int tipo, int rank){
		...
		if(tipo == 1){
			//cor1 = Color.CYAN;
			cor1 = (new Color(0,0,255,125)); // O terceiro campo, depois do campo B (RGB), refere-se à transparência
			switch(rank){
		...
		//Inimigo padrão mais forte e rápido
		if(tipo == 2 ){
			//cor1 = Color.BLUE;
		...
		//Inimigo padrão Lento, porém resistente
		if(tipo == 3){
			//cor1 = Color.GRAY;
		...
		x = Math.random() * PainelDoJogo.LARGURA / 2 + PainelDoJogo.LARGURA / 4; // Math.random() será a função usada
		y = -r;																// para que a coordenada seja aleatória
