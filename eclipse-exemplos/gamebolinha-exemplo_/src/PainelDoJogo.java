...
public class PainelDoJogo extends JPanel implements Runnable, KeyListener{
	...
	//Campos de variáveis
	public static int LARGURA = 400, ALTURA = 400; // static porque essa variável poderá ser 
	...
	public static ArrayList<Explosao> explosões;
	...
	private int bulletTimeDuração = 7000;
	...
	public PainelDoJogo(){
		...
		setFocusable(true); //faz com que seja possível o foco da tela
	...
	public void run(){
		 ...
		 g.setRenderingHint(
				 RenderingHints.KEY_ANTIALIASING,  // Anti-Análise da tela, jogador, inimigos, etc.
				 RenderingHints.VALUE_ANTIALIAS_ON	);
		 g.setRenderingHint(
				 RenderingHints.KEY_TEXT_ANTIALIASING, // Anti-Análise do texto
		 ...
		 explosões = new ArrayList<Explosao>();
		 ...
		 long targetTime = 1000 / FPS; // 1000 porque  são 1000 milissegundos, o que resulta em 1 segundo.
		 								//O resultado dará cerca de 33 milissegundos, ou seja,
		 								// ele rodará o loop em cada 33 milissegundos
		 
		 //Loop do jogo	 
		 while (Rodando){
			 
			 startTime = System.nanoTime(); // Função do java que pega o tipo de tempo em nanosegundos
			 
			 gameRefresh();
			 gameRender();
			 gameDraw();
			 
			 URDTimeMillis = (System.nanoTime() - startTime) / 1000000; // Sempre dividido por 1 milhão, pois queremos os milissegundos.
			 waitTime = targetTime - URDTimeMillis; // waitTime é assim pois precisa-se de um 
			 										//tempo de espera entre esses 33 milissegundos, 
			 										//o gameRender fará seu trabalho em 20 millis aprox, 
			 										//portanto, é necessário o restante do tempo como espera
			 try{
			 ...
			 if (frameCount == maxFrameCount){
				 averageFPS = 1000.0 / ((totalTime / frameCount) / 1000000); // Sempre dividido por 1 milhão, pois queremos os milissegundos.
				 frameCount = 0;
				 totalTime = 0;
			 }
		 }
	...
	private void gameRefresh(){ //Função que será responsável pelos refreshs e sua lógica
		...
		if(hordaTimerInicio == 0 && inimigos.size() == 0){
			...
			hordaDifTimerInicio = (System.nanoTime() - hordaTimerInicio) / 1000000; // Sempre dividido por 1 milhão, pois queremos os milissegundos.
		...
		for (int i = 0; i < explosões.size(); i++){
			boolean remove = explosões.get(i).Refresh();
			if(remove){
				explosões.remove(i);
				i--;
			}
		...
		for( int i = 0; i < inimigos.size(); i++){
			if(inimigos.get(i).estaMorto()){
				...
				double rand = Math.random(); // Função matemática que gera um número aleatório
				if (rand < 0.005)
				...
				in.explode();
				explosões.add(new Explosao(in.getX(),in.getY(),in.getR(),in.getR() + 30));
			}
		}
		...
		for (int i = 0; i < powerUps.size(); i++){
			...
			if ( dist < jR + r){
				...
				int nvPower = jogador.getNívelDoPoder();
		...
		if(bulletTimeTimer != 0){
			...
			if (bulletTimeTimerDif > bulletTimeDuração){
				bulletTimeTimer = 0;
				for (int i = 0; i < inimigos.size(); i++){
	...
	private void gameRender(){ // Função que será responsável pelo doublebuffering, ou seja, 
							   //pré-definir todos os sprites, renderizá-los e deixá-los prontos
		//render da tela
		g.setColor(new Color(110,115,250)); // (new Color(,,,)) é o parâmetro que se adiciona cores no padrão RGB
		g.fillRect(0, 0, LARGURA, ALTURA);
		g.setColor(Color.WHITE);
		
		//render efeito Bullet Time
		if(bulletTimeTimer != 0){
			g.setColor(new Color(255,255,255,64)); // o quarto elemento refere-se a transparência
			g.fillRect(0, 0, LARGURA, ALTURA);
		}
		...
		g.drawString("Nº de tiros: "+ tiros.size(),230,45);
		g.drawString("Nº de inimigos:"+ inimigos.size(), 230,35);
		...
		if(bulletTimeTimer != 0){
			...
			g.fillRect(20,60,(int) (100 - 100 * bulletTimeTimerDif / bulletTimeDuração), 8);
		}
		...
		for (int i = 0; i < jogador.getPoderNecessário(); i++){
	...
	private void gameDraw(){ // Função que será responsável por desenhar na tela
